...
Import BRL.MaxUnit
Import Database.OrmPostgreSQL

New TTestSuite.run()

Const ORM_PG_DB:String = "maxtest"
Const ORM_PG_HOST:String = "localhost"
Const ORM_PG_PORT:Int = 5432
Const ORM_PG_USER:String = "user"
Const ORM_PG_PASSWORD:String = "pass"

Function CreatePostgresOrm:TOrm()
	Return TOrm.Create("POSTGRESQL", ORM_PG_DB, ORM_PG_HOST, ORM_PG_PORT, ORM_PG_USER, ORM_PG_PASSWORD)
End Function

Function QuotePostgresIdentifier:String(name:String)
	Return "~q" + name.Replace("~q", "~q~q") + "~q"
End Function

Function DropOrmTable(orm:TOrm, tableName:String)
	Local query:TDatabaseQuery = orm.db.ExecuteQuery("DROP TABLE IF EXISTS " + QuotePostgresIdentifier(tableName) + " CASCADE")
	If query Then query.Free()
End Function

Function PrepareOrmTable(orm:TOrm, typeName:String, tableName:String)
	DropOrmTable(orm, tableName)
	orm.CreateTable(typeName)
End Function

Type TOrmPostgreSQLBasicCrudTest Extends TTest

	Method Test_SaveFindUpdateRemove_Works() { test }

		Local orm:TOrm = CreatePostgresOrm()

		Local users:TOrmDao<TOrmTestUser> = New TOrmDao<TOrmTestUser>.Init(orm, "TOrmTestUser")

		PrepareOrmTable(orm, "TOrmTestUser", "orm_test_users")
...
Type TOrmPostgreSQLQueryTest Extends TTest
...
Type TOrmPostgreSQLJsonTest Extends TTest
	...
	Method Test_JsonField_RoundTripsObject() { test }
		...
		PrepareOrmTable(orm, "TOrmJsonUser", "orm_json_users")
...
Type TOrmPostgreSQLTransactionTest Extends TTest
...
Type TOrmPostgreSQLSchemaValidationTest Extends TTest
	...
	Method Test_ValidateOnStartup_ReportsMissingTable() { test }
		...
		DropOrmTable(orm, "orm_test_users")
...
Type TOrmPostgreSQLIndexTest Extends TTest
	...
	Method Test_CreateTable_CreatesSingleColumnIndex() { test }
		...
		PrepareOrmTable(orm, "TOrmIndexedUser", "orm_indexed_users")
	...
	Method IndexExists:Int(orm:TOrm, tableName:String, indexName:String)
		...
		query.Prepare("SELECT indexname FROM pg_indexes WHERE schemaname = 'public' AND tablename = $1")
		query.AddString(tableName)
		query.Execute()

		While query.NextRow()
			If query.Value(0).GetString() = indexName Or query.Value(0).GetString().StartsWith(indexName) Then
				query.Free()
				Return True
			End If
		Wend
...
Type TOrmPostgreSQLFailureTest Extends TTest
	...
	Method Test_Update_WithoutPrimaryKey_Throws() { test }
		...
		PrepareOrmTable(orm, "TOrmNoPkEntity", "orm_no_pk_entity")
...
Type TOrmPostgreSQLPrimitiveRangeTest Extends TTest
	...
	Method Test_PrimitiveValues_RoundTrip() { test }
		...
		PrepareOrmTable(orm, "TOrmPrimitiveRangeEntity", "orm_primitive_range_entity")
...
Type TOrmPostgreSQLOrderLimitTest Extends TTest
...
Type TOrmPostgreSQLCustomJConvSerializerTest Extends TTest
	...
	Method Test_CustomJConvSerializer_RoundTripsJsonField() { test }
		...
		PrepareOrmTable(orm, "TOrmBoxEntity", "orm_box_entity")
...
Type TOrmPostgreSQLEnumTest Extends TTest
	...
	Method Test_NumericEnum_RoundTrips() { test }
		...
		PrepareOrmTable(orm, "TOrmEnumEntity", "orm_enum_entity")
...
Type TOrmPostgresPlaceholderRewriteTest Extends TTest

	Method Test_RewritesSinglePlaceholder() { test }
		AssertEquals("age > $1", Rewrite("age > ?"), "Single placeholder should rewrite to $1")
	End Method

	Method Test_RewritesMultiplePlaceholders() { test }
		AssertEquals("age > $1 AND name = $2", Rewrite("age > ? AND name = ?"), "Multiple placeholders should be numbered")
	End Method

	Method Test_EscapedQuestionMark_BecomesLiteralQuestionMark() { test }
		AssertEquals("metadata ? 'enabled'", Rewrite("metadata ?? 'enabled'"), "Escaped ?? should become literal ?")
	End Method

	Method Test_EscapedQuestionMarkAndPlaceholder() { test }
		AssertEquals("metadata ? $1", Rewrite("metadata ?? ?"), "Escaped ?? should not consume placeholder numbering")
	End Method

	Method Test_QuestionMarkInsideSingleQuotes_IsNotRewritten() { test }
		AssertEquals("name = '?' AND age > $1", Rewrite("name = '?' AND age > ?"), "Question mark inside string literal should not be rewritten")
	End Method

	Method Test_QuestionMarkInsideDoubleQuotes_IsNotRewritten() { test }
		AssertEquals("SELECT ~q?~q FROM users WHERE id = $1", Rewrite("SELECT ~q?~q FROM users WHERE id = ?"), "Question mark inside quoted identifier should not be rewritten")
	End Method

	Method Test_EscapedQuestionMarkInsideSingleQuotes_IsNotCollapsed() { test }
		AssertEquals("value = '??' AND id = $1", Rewrite("value = '??' AND id = ?"), "Escaped question marks inside string literal should remain unchanged")
	End Method

	Method Test_NumberingSkipsEscapedQuestionMarks() { test }
		AssertEquals("a ? b AND c = $1 AND d ? e AND f = $2", Rewrite("a ?? b AND c = ? AND d ?? e AND f = ?"), "Escaped question marks should not increment placeholder index")
	End Method

	Method Test_NoPlaceholders_ReturnsSameSql() { test }
		AssertEquals("SELECT * FROM users", Rewrite("SELECT * FROM users"), "SQL without placeholders should be unchanged")
	End Method

	Method Test_EmptyString_ReturnsEmptyString() { test }
		AssertEquals("", Rewrite(""), "Empty SQL should remain empty")
	End Method

	Method Rewrite:String(sql:String)
		Local dialect:TOrmPostgresDialect = New TOrmPostgresDialect
		Return dialect.RewritePlaceholders(sql)
	End Method
